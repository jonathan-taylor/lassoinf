...
$$
\frac{\alpha}{2} \|\beta\|^2_2.$$

This affects the quadratic part of the LASSO problem but of course it does not appreciably alter the variance of the score itself.

```{code-cell} ipython3
alpha = 0.5 * n
loss_noisy = 0.5 * cp.sum_squares(y_star - X_star @ beta_lasso) * f + alpha / 2 * cp.norm(beta_lasso, 2)**2
```
...
Q_hat = X_star.T @ X_star * f + alpha * np.eye(p)
```
