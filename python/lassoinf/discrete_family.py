...
def find_root(f, value, lower, upper, tol=1e-6):
    """
    Find root using bisection, robust to step functions.
    Assumes monotonic function crossing `value`.
    """
    if lower > upper:
        lower, upper = upper, lower
    
    f_lower = f(lower) - value
    f_upper = f(upper) - value
