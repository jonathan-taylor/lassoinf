...
$$
for 2-tensors $A$ and fixed vector fields $X, W$.
...
\label{lem:normal:taylor:tensor}
  Let $\partial h_i, 1 \leq i \leq d$ denote normal coordinate vector fields for
  a fixed orthonormal basis $E_{t^*}=\{E_{1,t^*},\dots, E_{d,t^*}\}$ of $T_{t^*}\mc{T}$ and let
  $$\mathfrak{a}=\nabla^2 \mu(\partial h_i, \partial h_j) + (y - \mu_t) \cdot g(\partial h_i, \partial h_j)_t$$
  for some $\mu \in C^{4+\alpha}(\mc{T},\R)$ with $\nabla \mu_{t^*}=0$. Then,
  \begin{equation}
\partial h_k(\nabla \mathfrak{a})_{t^*} = \nabla^3 \mu(\partial h_k, \partial h_i, \partial h_j)_{t^*}
    \end{equation}
  Further, up to a universal constant $\bar{C}$
  \begin{equation}
    \left|\mathfrak{a}_t - \mathfrak{a}_{t^*} - \nabla^3 \mu(\log_{t^*}(t), \partial h_i, \partial h_j)_{t^*}
    \right| \leq \bar{C} \cdot d(t^*,t)^2 \inf_{E_{t^*}} \sup_{t \in B_{t^*}(r)} \| \nabla^2 \mathfrak{a}^{E_{t^*}}_t\|.
    \end{equation}
...
& \qquad  \frac{1}{2}\sum_{i=1}^d \biggl[ (\nabla^2 \mu(\log_{t^*}(t), E_{i,t^*}))^2 + \\
    & \qquad \qquad 
      \nabla^3 \mu_{t^*}(\log_{t^*}(t), \log_{t^*}(t), E_{i,t^*}) \nabla^2 \mu_{t^*}(E_{i,t^*}, \log_{t^*}(t)) \biggr] \biggr| \\
    & \qquad \leq \bar{C} d(t^*,t)^{3+\alpha} \inf_{E_{t^*}}\sup_{t \in B_{t^*}(r)} C_{\alpha,E_{t^*}}(\bar{\mu}^{E_{t^*}}, r)
\end{aligned}
    \end{equation}
The Taylor expansion does not depend on the choice of orthonormal basis.
  \end{lemma}

\jt{Not sure of the exponent in the H\"older..., I think it should $3+\alpha$ because we only went up to order 3, as the function $\mathfrak{g}$ should only have 3 derivatives.}
...
We might as well use normal coordinates $(\partial u_i)_{1 \leq i \leq d}$ at $t^*$ with basis $E_{t^*}=\{E_{1,t^*}, \dots, E_{d,t^*}\}$.
...
$$
\begin{aligned}
  \nabla (\det A)(\partial u_k)_{t^*} &= \det(A_{t}) \cdot \text{Tr}\left(\left(\frac{\partial \log \det}{\partial M}\biggl|_{M=H_t}\right) \nabla H(\partial u_k)_t - \left(\frac{\partial \log \det}{\partial M}\biggl|_{M=\Lambda_t}\right) \nabla \Lambda(\partial u_k)_t\right) \\
  &= \det(H_{t^*}) \cdot \left(\sum_{i,j} \nabla^3 \mu(\partial u_k, \partial u_i, \partial u_j)_{t^*} (H_{t^*}^{-1})_{ji} \right)
\end{aligned}
$$
...
\end{aligned}
 $$
Assuming that $\|\nabla^3 \mu\|$ and $\|\nabla^4 \mu\|$ grow no faster than $\nabla^2 \mu$ the terms in the
Hessian, after dividing through by  $\det(A_t)$  will remain bounded.
We can conclude that
$$
\left|\det( A_t) - \det (A_{t^*}) - \det(A_{t^*}) \left( \sum_{i,j} \nabla^3 \mu_{t^*}(\log_{t^*}(t), E_{i,t^*}, \partial E_{j,t^*}) (H_{t^*}^{-1})_{ji}\right) \right| \leq \det(H_{t^*}) d(t^*,t)^2 C(\mu).
$$
...
&  \leq \qquad (1 + d(t^*,t)^{3+\alpha} C(\mu)) \cdot \exp\left(-\frac{1}{2} \sum_{i=1}^d \nabla^2 \mu_{t^*}(\log_{t^*}(t), E_{i,t^*})^2\right) .
\end{aligned}
\end{equation}
