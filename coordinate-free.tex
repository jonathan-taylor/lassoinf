...
terms with $\nabla^3 \mu(\cdot) \nabla^2 \mu(\cdot)$. As we have only assumed $\mu \in C^{4+\alpha}$ we will have to consider these terms separately. On the other hand, $\nabla \mu$ can only grow to size $\delta^{-1}_{t^*}(-\log (\delta_{t^*}))$ on our ball.
