...
\begin{document}
	...
	The proof of Lemma~\ref{lem:approximation-peak-intensity} is given in Section~\ref{subsec:pf-approximation-peak-intensity}. The restriction that $y \geq \mu_{t^*} - o(\delta_{t^*}^{-1})$ is made so that $\nabla^2 Y_t$ is negative definite with very high probability,  conditional on $Y_t = y$.
        ...
        \jt{Is \eqref{eqn:hessian-deterministic-equivalent-derivative} correct? I don't see why there is a $(y-\mu_{t^*})$ in front of the term $\dot{\Lambda}_{t^*}\Lambda_{t^*}^{-1} \mu_{t^*}h$. Shouldn't this come from differentiating
          $\Gamma_t(\nabla \mu_t)$? In Riemannian notation, this would come from differentiating the Riemannian Hessian of $\mu$ and would have no $(y-\mu_{t^*})$ term...}

        \jt{What argument do you want to have to $\delta_{t^*}$ above? I am presuming you mean $\delta_{t^*}(u_{t^*})$.}
